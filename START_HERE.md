...
后续使用时，最省事的方式是让 AI 读取 `START_PROMPT.md`。

`START_PROMPT.md` 会指向 `START_PROJECT.md`。AI 读取后会完成初始化检查，然后等待你继续提供原型地址、需求描述、数据库连接信息或其它输入源。

本文件保留为模板说明入口。

如果这是第一次启动新项目，先查看 `FIRST_RUN_CHECKLIST.md`，确认模板没有真实需求、任务、报告或运行时产物。
